...
final class MissionsTests: XCTestCase {
    ...
    func testTodaysChallengeSeedMatchesUTCGoldens() async {
        let store = ProfileStore(testing: Profile())
        // Goldens pinned in DailyChallengeTests (layoutVersion 1).
        XCTAssertEqual(store.todaysChallengeSeed(now: utc(2026, 6, 10)), 0xFBC0_C337_38F0_2209)
        XCTAssertEqual(store.todaysChallengeSeed(now: utc(2026, 6, 11)), 0x9399_C653_92BA_DC4F)
