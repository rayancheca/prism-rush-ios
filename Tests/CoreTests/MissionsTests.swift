...
final class MissionsTests: XCTestCase {
    ...
    func testTodaysChallengeSeedMatchesUTCGoldens() async {
        let store = ProfileStore(testing: Profile())
        // Goldens pinned in DailyChallengeTests (layoutVersion 12 as of v2.3 — the Warden arena
        // grown 660 → 770 m to hold a longer fight). These must track that file: the daily
        // leaderboard is only meaningful if every player on a date gets the same track from the same
        // layout version. This is the pin that is easy to miss — it lives in the META layer and
        // asserts the same numbers from the other side of the seam.
        XCTAssertEqual(store.todaysChallengeSeed(now: utc(2026, 6, 10)), 0x03B5_B844_D08B_98AF)
        XCTAssertEqual(store.todaysChallengeSeed(now: utc(2026, 6, 11)), 0x440D_2303_981F_5A63)
